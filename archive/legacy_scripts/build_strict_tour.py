...
<head>
  ...
  <style>
    /* =========================================================================
       HERITAGE LUXURY RESET & ARCHITECTURAL CANVAS
       ========================================================================= */
    *, *::before, *::after {
      margin: 0;
      padding: 0;
    ...
    /* SCROLL CONTAINER: 1100vh for ultra-smooth, responsive, buttery scrubbing */
    .scroll-container {
      width: 100%;
      height: 1100vh;
      position: relative;
    ...
    /* 3D DRONE CAMERA RIG (Organic banking, roll, and elevation) */
    .camera-rig {
      position: absolute;
      inset: 0;
      width: 100%;
    ...
    /* MEDIA BOX (RESPONSIVE RETINA COVER) */
    .media-box {
      position: absolute;
      top: 50%;
      left: 50%;
    ...
    /* =========================================================================
       SCENE 2: SUB-PIXEL 3D DOOR ASSEMBLY
       Coordinates on 1376x768 main2.jpg:
       Left: 648px (47.093023%)
       Top: 485px (63.151042%)
       Width: 80px (5.813953%)
       Height: 87px (11.328125%)
       Center X: 688px (50.000000%), Center Y: 528.5px (68.815104%)
       ========================================================================= */
    .door-assembly {
      position: absolute;
      left: 47.093023%;
    ...
    .door-leaf.left {
      ...
      background-image: url('img/door_left.png?v=5');
    }

    .door-leaf.right {
      right: 0;
      transform-origin: right center;
      background-image: url('img/door_right.png?v=5');
    }

    /* NATURAL SUNSET/PASTEL WINDOW GLOW ON SCENE 3 */
    .window-light-burst {
      position: absolute;
      inset: 0;
      background: radial-gradient(circle at 71% 48%, rgba(255, 238, 205, 0.85) 0%, rgba(255, 215, 155, 0.35) 42%, transparent 72%);
      opacity: 0;
      pointer-events: none;
    ...
    /* LIVING ATMOSPHERE (Canvas Drifting Rose Petals & Golden Spores) */
    #particles-canvas {
      position: absolute;
      inset: 0;
      width: 100%;
    ...
    /* SOFT CINEMATIC VIGNETTE (Pastel Charcoal) */
    .cinema-vignette {
      position: absolute;
      inset: 0;
      pointer-events: none;
      z-index: 18;
      background: radial-gradient(circle at 50% 50%, transparent 68%, rgba(7, 5, 4, 0.40) 100%);
    }
    ...
    /* SUBTLE SCROLL HINT (Fades out immediately upon interaction) */
    .scroll-hint {
      position: fixed;
      bottom: 28px;
      left: 50%;
...
<body>
    ...
    <div class="camera-rig" id="camera-rig">
      ...
      <section class="tour-scene" id="scene-1" style="opacity: 1; z-index: 1;">
        <div class="media-box" id="box-1">
          <img class="bg-img" src="img/main1.jpg?v=5" alt="Palace Roof & Twilight Sky" id="img-1" />
        </div>
      </section>

      <!-- SCENE 2: PALACE COURTYARD & 3D ENTRANCE DOORS (main2_open.jpg: 1376x768) -->
      <section class="tour-scene" id="scene-2" style="z-index: 2;">
        <div class="media-box" id="box-2">
          <img class="bg-img" src="img/main2_open.jpg?v=5" alt="Palace Courtyard & Facade" id="img-2" />
      ...
      <section class="tour-scene" id="scene-3" style="z-index: 3;">
        <div class="media-box" id="box-3">
          <img class="bg-img" src="img/reception.jpg?v=5" alt="Reception Lobby & Arch Window" id="img-3" />
          <div class="window-light-burst" id="window-burst"></div>
        </div>
      </section>
      ...
      <section class="tour-scene" id="scene-4" style="z-index: 4;">
        <div class="media-box" id="box-4">
          <img class="bg-img" src="img/banqent.jpg?v=5" alt="Banquet Night Exterior Entrance" id="img-4" />
        </div>
      </section>

      <!-- SCENE 5: ROYAL ELEPHANT FLORAL WALKWAY (banq1.jpg: 1024x1024) -->
      <section class="tour-scene" id="scene-5" style="z-index: 5;">
        <div class="media-box" id="box-5">
          <img class="bg-img" src="img/banq1.jpg?v=5" alt="Floral Elephant Walkway" id="img-5" />
        </div>
      </section>
      ...
      <section class="tour-scene" id="scene-6" style="z-index: 6;">
        <div class="media-box" id="box-6">
          <img class="bg-img" src="img/banq2.jpg?v=5" alt="Grand Ballroom Chandeliers" id="img-6" />
        </div>
      </section>
      ...
      <section class="tour-scene" id="scene-7" style="z-index: 7;">
        <div class="media-box" id="box-7">
          <img class="bg-img" src="img/banq3.jpg?v=5" alt="Center Aisle Perspective" id="img-7" />
        </div>
      </section>
      ...
      <section class="tour-scene" id="scene-8" style="z-index: 8;">
        <div class="media-box" id="box-8">
          <img class="bg-img" src="img/banq4.jpg?v=5" alt="Royal Wedding Mandap Finale" id="img-8" />
        </div>
      </section>
  ...
  <script>
    (function () {
      ...
      const windowBurst = document.getElementById('window-burst');
      ...
      // =========================================================================
      // VIRTUAL TOUR CHOREOGRAPHY ENGINE (THE STRICT 8-STAGE SEQUENCE)
      // =========================================================================
      function renderTour(p) {
        progressFill.style.width = (p * 100).toFixed(2) + '%';

        // -----------------------------------------------------------------------
        // STAGE 1: [0.00 -> 0.14] START AT main1.jpg (Roof pavilion & twilight sky)
        //          Drone camera descends vertically along the facade into courtyard
        // -----------------------------------------------------------------------
        if (p <= 0.15) {
        ...
        //          Drone pushes towards central entrance; double doors swing inward in 3D
        // -----------------------------------------------------------------------
        if (p >= 0.065 && p <= 0.29) {
        ...
        //          Glides into reception, then camera angle changes to the big window
        //          covered with semi-transparent curtains, zooms deep into it,
        //          and zoops out through the window!
        // -----------------------------------------------------------------------
        if (p >= 0.215 && p <= 0.43) {
          ...
          // Phase A (0.22 -> 0.29): fly into reception hall
          // Phase B (0.29 -> 0.41): turn and push deep into the big curtained window
          // Window center in reception.jpg is at X: 71%, Y: 48%
          const winProg = norm(p, 0.28, 0.41);
          const winSmooth = easeInOutQuad(winProg);

          const scale = 1.02 + norm(p, 0.22, 0.29) * 0.25 + winSmooth * 2.8;
          const panX = -(winSmooth * 21.0);
          const panY = -(winSmooth * 4.0);

          boxes[3].style.transformOrigin = '71.0% 48.0%';
          boxes[3].style.transform = 'translate(-50%, -50%) translate3d(' + panX + '%, ' + panY + '%, 0) scale(' + scale + ')';

          // Warm evening light burst as drone reaches window glass
          const glowAlpha = Math.sin(norm(p, 0.32, 0.41) * Math.PI) * 0.75;
          windowBurst.style.opacity = glowAlpha.toFixed(3);
        } else {
        ...
        // Drone Camera Banking & Elevation:
